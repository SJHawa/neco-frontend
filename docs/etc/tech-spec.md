...
이 문서는 `Relay Coding Server API Specification v1.2`와 `/Users/admin/Desktop/study/neconaeco/backend/docs/specs`의 확정 계약을 기준으로 프론트엔드 구현에 필요한 기술 설계를 정리한다.

프론트엔드는 AI 채팅을 중심으로 방 생성, 초대 확인, 방 참가, 게임 시작 준비를 처리하고, 게임 시작 이후에는 Socket.IO 이벤트를 기준으로 실시간 릴레이 코딩 게임 상태를 동기화한다.

`docs/etc/api-spec.md`, `docs/etc/sequence-diagram.md`에 있던 계약과 흐름도 이 문서에 통합 반영한다. 문서 간 충돌 시에는 백엔드 spec hierarchy를 우선한다.
...
- `/rooms/:gameRoomId/play` 진입 시 local game state가 없으면 `join-room` 이후 수신되는 서버 이벤트를 authoritative source로 사용한다.
- WebSocket 연결이 끊기면 자동 재연결이나 자동 복구를 시도하지 않는다. MVP에서는 새 초대 없이 재참여할 수 없다는 백엔드 정책을 따른다.
...
- 별도 participant mutation API를 직접 호출하는 흐름은 MVP 범위에서 가정하지 않는다.
...
### ROOM_CREATE 2단계 상호작용

방 생성은 sequence 기준으로 다음 2단계 상호작용으로 처리한다.

1. 사용자가 방 생성 의도를 보내면 서버는 먼저 난이도 선택 프롬프트를 반환한다.
2. 사용자가 난이도를 선택하면 서버는 해당 난이도의 미션 템플릿 후보를 반환한다.
3. 사용자가 템플릿을 확정하면 실제 방이 생성되고 room waiting state로 전환한다.

프론트는 별도 `clientAction` 없이 카드 클릭 결과를 자연어 또는 규칙화된 메시지 문자열로 다시 전송한다.
...
`commandResult.status = PENDING`이고 난이도 선택 프롬프트가 오면 난이도 선택 UI를 표시한다.

난이도 선택 이후 `assistantMessage.metadata.templates`가 있으면 미션 템플릿 선택 UI를 표시한다.
...
- `POST /game-rooms/{gameRoomId}/start` 성공만으로는 라우팅하지 않는다.
...
- 변경분을 debounce 후 `code-change` payload로 직렬화해 Socket.IO로 전송한다.
- `code-updated` 수신 시 본인이 보낸 변경이 아니면 editor document에 적용한다.
- 현재 연결의 `socket.id` 또는 별도 client change id를 기준으로 중복 반영을 방지한다.
- turn 종료 시 authoritative 저장본은 `turn-submit.codeSnapshot` 기준으로 처리한다.
...
1. `currentRoom`이 있고 내 `membershipStatus = JOINED`이며 방 상태가 `WAITING` 또는 `IN_PROGRESS`이면 Socket.IO를 연결한다.
2. 연결이 새로 만들어질 때마다 `join-room` 이벤트를 전송한다.
3. MainPage room waiting state와 RoomPage는 같은 room-scoped 연결 정책을 사용한다.
4. `room-participants-updated`로 대기 상태를 동기화하고, `game-started`와 `game-state-updated`로 게임 진입 상태를 반영한다.
5. 플레이 중에는 `code-change`, `turn-submit`을 송신하고 `code-updated`, `turn-evaluated`, `turn-changed`, `mission-result`를 수신한다.
6. 라우트 전환으로 같은 방을 계속 보고 있으면 기존 연결을 재사용할 수 있다. 다만 소켓 인스턴스가 다시 만들어지면 `join-room`도 다시 전송한다.
7. 사용자가 room-scoped 화면을 벗어나거나 인증이 해제되면 연결을 종료한다.

### 재연결 정책

- MVP에서는 reconnect-and-resume를 지원하지 않는다.
- 연결이 끊기면 해당 참가자는 `LEFT`로 처리될 수 있으며, 현재 턴 담당자였다면 `TIMEOUT` 종료가 뒤따를 수 있다.
- 프론트는 exponential backoff 기반 자동 재연결을 시도하지 않는다.
- 연결 종료 시에는 세션이 종료되었음을 안내하고, 안전한 화면 상태로 복귀시킨다.
- 새 초대 없이 같은 방으로 자동 복귀하는 흐름은 지원하지 않는다.
...
- `POST /start` 응답만으로는 이 처리를 선행하지 않는다.
...
- 별도 결과 조회 API가 없으므로 ResultPage는 in-memory event state를 기본 전제로 한다.
...
- Socket.IO 연결 종료 시 세션 종료 안내와 복귀 동선을 표시한다.
...
current room status IN_PROGRESS
  -> auto-resume를 가정하지 않고 비정상 진입 여부를 확인한 뒤 안전한 복귀 UX를 제공
...
## 32. Resolved Questions

1. **AI chat session 생성 시점** — 회원가입 API(`POST /auth/register`) 처리 시 서버가 자동 생성한다.

2. **`game-started` 에디터 초기 파일 정보** — `missionState.projectStructure.files` 각 요소에 `fileUrl: string` (S3 URL)을 추가한다. 프론트는 해당 URL로 초기 코드를 fetch하여 에디터에 로드한다.

3. **코드 동기화 payload 기준** — `content` (전체 파일 내용) 기준으로 확정한다. `code-change` / `code-updated` 이벤트 모두 `content` 필드를 사용한다.

4. **`GET /game-rooms` 단일 방 보장 조건** — `status`가 `WAITING`인 방에 한해 한 사용자당 하나임을 서버가 보장한다.

5. **`IN_PROGRESS` 방 초기 진입 처리** — `GET /game-rooms`는 `WAITING` 상태만 반환하므로 고려할 필요 없다.

6. **미션 템플릿 선택 전달 방식** — 템플릿 카드 클릭 시 자연어 메시지(예: "이 템플릿으로 할게")를 전송한다.

7. **AI 채팅 메시지 pagination** — 불필요하다. 현재 명세 유지.

8. **토큰 저장 위치** — access token: `sessionStorage`, refresh token: `localStorage`.

9. **passwordHash 생성 방식** — 프론트에서 SHA-256으로 해시하여 전송한다. 서버는 별도 hashing 없이 그대로 사용한다. (HTTPS 전제)

10. **실패 메시지 UI 패턴** — AI 채팅 말풍선으로만 표시한다.
